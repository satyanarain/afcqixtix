...
function manageLoginLog($usrid) {
   $lastlogin = DB::table('user_login_log')
           ->whereDate('login_date', DB::raw('CURDATE()'))
           ->where([['user_id', $usrid]])
           ->count();
   if ($lastlogin == 0) {
       DB::table('user_login_log')->insert(array('user_id' => $usrid, 'login_date' => now()));
   } else {
       DB::table('user_login_log')
               ->whereDate('login_date', DB::raw('CURDATE()'))
               ->where([['user_id', $usrid]])
               ->update(['login_date' => now()]);
   }
   return true;
}

function manageLogoutLog($usrid) {
   $ua = getBrowser();
   $yourbrowser = "Your browser: " . $ua['name'] . " " . $ua['version'] . " Operating System: " . $ua['platform'];
   DB::table('user_login_log')
           ->whereDate('login_date', DB::raw('CURDATE()'))
           ->where([['user_id', $usrid]])
           ->update(['logout_date' => now(), 'os_browse_data' => $yourbrowser]);

   return true;
}

function lastLogin($usrid) {
   $datauser = DB::table('user_login_log')
                   ->where([['user_id', $usrid]])->orderby('log_id', 'desc')
                   ->skip(1)->first();
   return $datauser->login_date;
}
